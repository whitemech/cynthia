...
ForwardSynthesis::Search::Search(Problem& problem, ForwardSynthesis& forwardSynthesis)
    : problem_{problem}, init_node{problem.formula}, forwardSynthesis_{forwardSynthesis}{}

std::vector<ForwardSynthesis::SearchConnector> ForwardSynthesis::Search::expand_(SearchNode* node){
  auto sdd = node->sdd;
  std::vector<ForwardSynthesis::SearchConnector> connectors;
    ...
    for (const auto& pair : new_children) {
      ...
      if (env_state_node.get_type() != SddNodeType::ENV_STATE) {
        auto formula_next_state = forwardSynthesis_.next_state_formula_(env_state_node.get_raw());
        auto sdd_next_state = forwardSynthesis_.formula_to_sdd_(formula_next_state);
        ...
        env_child.first = forwardSynthesis_.formula_to_sdd_(problem_.ast_manager->make_tt());;
        env_child.second = sdd_next_state;
        env_children.emplace_back(env_child);
        ForwardSynthesis::SearchConnector connector = ForwardSynthesis::SearchConnector(problem_, node, env_children);
        ...
        for (; env_child_it != env_children_end; ++env_child_it) {
          bool ignore = false;
          auto env_node = SddNodeWrapper(env_child_it.get_prime(), problem_.manager);
          auto state_node = SddNodeWrapper(env_child_it.get_sub(), problem_.manager);
          std::pair<SddNodeWrapper, SddNodeWrapper> env_child;
...
strategy_t ForwardSynthesis::Search::do_search_(const logic::ltlf_ptr& formula, std::set<SddSize>& path) {
