...
ltlf_ptr simplify(const LTLfImplies& formula) {
  ...
  std::transform(formula.args.begin(), formula.args.end() - 1,
                 new_container.begin(), [](const ltlf_ptr& formula) {
                   return formula->ctx().make_not_unified(formula);
                 });
...
ltlf_ptr simplify(const LTLfEquivalent& formula) {
  ...
  std::transform(
      formula.args.begin(), formula.args.end(), args_negative.begin(),
      [](const ltlf_ptr& arg) { return arg->ctx().make_not_unified(arg); });
